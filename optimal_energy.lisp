(defun energy-exp (x0 y0 xf yf u v sensor-on)
  ...
  (let*
   ...
   ((c0 0.1473)
    ...
    ;; Voltage to thruster motor, [V]
    ;; 	+ used to convert [A] to [W] for motor input command
    (volts 12)
    ...
    ;; new waypoint information calculated such that glider arrives at target location
    ;; 	+ glider only dead reckoning with respect to its thruster velocity
    (way-dist nil)
    (way-time nil)
    (way-x nil)
    (way-y nil))
  ...
  (setf ground-v (get-ground-v opt-v v-c-para v-c-perp))

  ;; calculate new waypoint information to be commanded to the glider
  (setf way-dist (mag (- xf x0) (- yf y0)))
  (setf way-time (/ way-dist ground-v))
  (setf way-x (/ (* opt-v-x (* opt-v way-time)) opt-v))
  (setf way-y (/ (* opt-v-y (* opt-v way-time)) opt-v))

  ;; helpful print statement for when commanding glider
  (format t "Initial Conditions: ~%")
  (printem "initial x position:   " x0 "[m]") (format t "~%")
  (printem "initial y position:   " y0 "[m]") (format t "~%")
  (printem "target x position:    " xf "[m]") (format t "~%")
  (printem "target y position:    " yf "[m]") (format t "~%")
  (printem "distance to waypoint: " way-dist "[m]") (format t "~%")
  (printem "x ocean currents:     " u "[m/s]") (format t "~%")
  (printem "y ocean currents:     " v "[m/s]") (format t "~%")
  (format t "~%Energy Optimization: ~%")
  (printem "optimal velocity:     " opt-v "[m/s]") (format t "~%")
  (printem "ground speed:         " ground-v "[m/s]") (format t "~%")
  (printem "time to complete:     " way-time "[s]") (format t "~%")
  (printem "optimal thruster A:   " opt-i "[A]") (format t "~%")
  (printem "optimal thruster W:   " (* opt-i volts) "[W]") (format t "~%")
  (printem "new waypoint x:       " way-x "[m]") (format t "~%")
  (printem "new waypoint y:       " way-y "[m]") (format t "~%")
...
;; get the ground velocity of the glider, [m/s]
(defun get-ground-v (opt-v v-c-para v-c-perp)
  (+ v-c-para (expt (- (expt opt-v 2) (expt v-c-perp 2)) 0.5)))
...
(defun objective (i hotel buoyancy-engine v-c-para v-c-perp c0 c1 c2)
  (/ (+ hotel buoyancy-engine i) 
     (+ v-c-para (expt (- (expt (get-v i c0 c1 c2) 2) (expt v-c-perp 2)) 0.5))))
...
(defun get-heading (x0 y0 x1 y1)
  (if (< (atan (- y1 y0) (- x1 x0)) 0) 
      (+ (* 2 pi) (atan (- y1 y0) (- x1 x0))) 
      (atan (- y1 y0) (- x1 x0))))
...
(defun mission()
  ;; use deg2utm function in matlab to get coordinates in x,y system
  ;; functional form: (energy-exp x1 y1 x2 y2 u v t)

  (format t "~%~%>> MISSION 1: (test)~%")
  (energy-exp 0 0 1 1 0.4 0.2 t))
